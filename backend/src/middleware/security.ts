...
// 🔒 安全性改進：批量操作速率限制（防止資源耗盡攻擊）
export const bulkOperationRateLimit = rateLimit({
  windowMs: 60 * 60 * 1000, // 1小時
  max: 5, // 每小時最多5次批量匯入
  message: {
    error: '批量操作過於頻繁，請等待1小時後再試',
    retryAfter: 60 * 60
  },
  standardHeaders: true,
  legacyHeaders: false,
  keyGenerator: (req: Request) => {
    return `bulk-${getClientIP(req)}`
  }
})

// 🔒 安全性改進：密碼操作速率限制（防止暴力破解和資源耗盡）
export const passwordOperationRateLimit = rateLimit({
  windowMs: 60 * 60 * 1000, // 1小時
  max: 5, // 每小時最多5次密碼操作
  message: {
    error: '密碼操作過於頻繁，請等待1小時後再試',
    retryAfter: 60 * 60
  },
  standardHeaders: true,
  legacyHeaders: false,
  keyGenerator: (req: Request) => {
    // 基於 IP 和目標用戶 ID 的組合限制
    const targetId = req.params?.id || 'self'
    return `pwd-${getClientIP(req)}-${targetId}`
  }
})

...
// 新增客訂單
router.post('/', authenticate, customerOrderValidation, handleValidationErrors, async (req: AuthenticatedRequest, res: Response) => {
  try {
  ...
  try {
    ...
    if (user.role === 'admin') {
      ...
      if (userAccessibleStores.length > 0) {
        // ⚡ 效能優化：使用單一查詢取得所有可訪問分店的客訂單（避免 N+1）
        orders = await db.getCustomerOrdersByStoreIds(userAccessibleStores)
      }
    }
...
// 獲取特定客訂單
router.get('/:id', authenticate, requirePermission('view_payments'), async (req: AuthenticatedRequest, res: Response) => {
  try {
...
// 更新客訂單狀態（完成/進行中）
router.patch('/:id/status', authenticate, requirePermission('edit_payments'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const orderId = parseInt(req.params.id)
    if (isNaN(orderId)) {
      res.status(400).json({ message: '無效的客訂單ID' })
...
// 刪除客訂單
router.delete('/:id', authenticate, requirePermission('delete_payments'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const orderId = parseInt(req.params.id)
    if (isNaN(orderId)) {
      res.status(400).json({ message: '無效的客訂單ID' })

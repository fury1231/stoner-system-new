  ...
  <div class="space-y-6">
    ...
    <div v-else>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="user in users" :key="user.id" class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-6 py-4 text-sm text-gray-900">
                  <div class="max-w-xs truncate" :title="getPermissionLabels(user.permissions)">
                    {{ user.permissions.length > 0 ? getPermissionLabels(user.permissions) : '無' }}
                  </div>
                </td>
      ...
      <div class="md:hidden space-y-4">
        <div v-for="user in users" :key="user.id" class="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-md transition-shadow">
        ...
        <!-- 空狀態 -->
        <div v-if="users.length === 0" class="bg-white rounded-lg border border-gray-200 p-8 text-center">
...
defineEmits<{
  (e: 'addUser'): void
  (e: 'editUser', user: UserRecord): void
  (e: 'changePassword', user: UserRecord): void
...
const getPermissionLabels = (permissions: string[]): string => {
  return permissions.map(p => props.availablePermissions[p] || p).join(', ')
}
</script>

...
// 獲取所有用戶
router.get('/', authenticate, requirePermission('manage_users'), async (req: Request, res: Response): Promise<void> => {
  try {
...
// 獲取特定用戶
router.get('/:id', authenticate, requirePermission('manage_users'), async (req: Request, res: Response): Promise<void> => {
  try {
...
// 創建新用戶
router.post('/', authenticate, requirePermission('manage_users'), userValidation, async (req: Request, res: Response): Promise<void> => {
  try {
...
// 更新用戶
router.put('/:id', authenticate, requirePermission('manage_users'), userUpdateValidation, async (req: Request, res: Response): Promise<void> => {
  try {
    const errors = validationResult(req)
    if (!errors.isEmpty()) {
    ...
    // 防止管理員停用自己
    if (typeof is_active !== 'undefined' && is_active === false && (req as any).user.username === existingUser.username) {
      res.status(400).json({ message: '無法停用自己的帳戶' })
      return
    }
    
    // 防止非管理員停用管理員帳戶
    const currentUser = await db.getUserByUsername((req as any).user.username)
    ...
    if (username && username !== existingUser.username) {
      const userWithSameName = await db.getUserByUsername(username)
      if (userWithSameName) {
        res.status(400).json({ message: '用戶名已存在' })
        return
      }
    ...
    // 不返回密碼哈希
    const { password_hash, ...safeUser } = updatedUser
    console.log('Returning updated user data:', safeUser.id, safeUser.username)
    res.json(safeUser)
  } catch (error) {
...
// 刪除用戶
router.delete('/:id', authenticate, requirePermission('manage_users'), async (req: Request, res: Response): Promise<void> => {
  try {
    const id = parseInt(req.params.id!)
    if (isNaN(id)) {
      res.status(400).json({ message: '無效的用戶ID' })
    ...
    // 防止用戶刪除自己
    if ((req as any).user.username === user.username) {
...
// 管理員重設用戶密碼
router.post('/:id/admin-reset-password', authenticate, requirePermission('manage_users'), [
  body('newPassword')
    .isLength({ min: 8, max: 128 })
  ...
  try {
    ...
    // 更新密碼
    const updated = await db.updateUser(id, { password_hash: passwordHash })
    if (!updated) {
...
router.post('/change-password', authenticate, [
  ...
  try {
    ...
    // 更新密碼
    const updated = await db.updateUser(user.id, { password_hash: newPasswordHash })
    if (!updated) {

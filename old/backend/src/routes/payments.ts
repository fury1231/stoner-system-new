...
router.post('/', authenticate, paymentValidation, handleValidationErrors, async (req: Request, res: Response) => {
  try {
...
router.get('/', authenticate, requirePermission('view_payments'), async (req: Request, res: Response) => {
  try {
    ...
    if (req.query.payment_method && typeof req.query.payment_method === 'string') {
      if (['現金', '匯款', '電子支付', '店內支出', '提領', '客訂單'].includes(req.query.payment_method)) {
    ...
    if (req.query.payment_methods) {
      ...
      const validMethods = methods.filter(method => 
        ['現金', '匯款', '電子支付', '店內支出', '提領', '客訂單'].includes(method)
      )
      
      if (validMethods.length > 0) {
        filters.payment_methods = validMethods
      }
    }
...
router.get('/:uuid', authenticate, requirePermission('view_payments'), async (req: Request, res: Response) => {
  try {
    ...
    // 檢查用戶是否有權限查看這個分店的記錄
    if (user.store_id !== null && user.store_id !== undefined && user.store_id !== payment.store_id) {
...
router.put('/:uuid', authenticate, requirePermission('edit_payments'), paymentUpdateValidation, handleValidationErrors, async (req: Request, res: Response) => {
  try {
    const uuid = req.params.uuid
    if (!uuid) {
      res.status(400).json({ message: 'UUID 參數是必需的' })
    ...
    // 檢查用戶是否有權限編輯這個分店的記錄
    if (user.store_id !== null && user.store_id !== undefined && user.store_id !== oldPayment.store_id) {
      res.status(403).json({ message: '權限不足：無法編輯其他分店的記錄' })
      return
    }
    ...
    const changedFields = Object.keys(updates).filter(key =>
      oldPayment[key as keyof typeof oldPayment] !== updates[key]
    )
...
router.delete('/:uuid', authenticate, requirePermission('delete_payments'), async (req: Request, res: Response) => {
  try {
    const uuid = req.params.uuid
    if (!uuid) {
      res.status(400).json({ message: 'UUID 參數是必需的' })
...
router.delete('/', authenticate, requirePermission('delete_payments'), async (req: Request, res: Response) => {
  try {
    ...
    if (user.role !== 'admin') {
      ...
      // ⚡ 效能優化：批量查詢所有記錄，避免 N+1 問題
      const payments = await db.getPaymentsByUuids(uuids)

      // 過濾出用戶有權限刪除的記錄
      const allowedUUIDs = payments
        .filter(payment => userAccessibleStores.includes(payment.store_id))
      ...
      const deletedCount = await db.batchDeletePayments(allowedUUIDs)
...
router.post('/bulk-import', authenticate, requirePermission('manage_payments'), async (req: Request, res: Response) => {
  try {
...
// 測試數據生成端點（僅用於開發環境）
router.post('/generate-test-data/:storeId', authenticate, requirePermission('edit_payments'), async (req: Request, res: Response) => {
  try {
